...
or errors. The scripts contain 106 native `assert` statements, not a measured execution
...
passed with 53 server completion records — the newest being the two refusals of an unsupported container
type — and 13 client records; the log carries exactly one addon `(from …)` frame, the deliberate
