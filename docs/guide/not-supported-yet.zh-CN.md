...
两个玩家看同一个菜单时，**看不到同一份箱子**：`static` 菜单的库存是每个会话一份的（每个人自己的真实库存），这是刻意的。

"共享货架"要用**图标**来表达：货架上的东西是页面的布局和 `override slot` 放的，`override slot` 与
`updateIconForKey` 会把改动推给正在看这一页的**所有**玩家。所以商店 = 有图标的共享货架 + 空格子的各人存储。
真的让多人操作同一个真实库存，会和"每个会话一份、每会话一页"的模型正面冲突，我们不打算做。
...
- **拖拽与 `locked icons`**：`12-client-drag.sk` 发的是真正的拖拽包（拖拽是一串"把阶段放在 button 里"
  ...
  数字键。
- **容器下方那几行**：`11-client.sk` 专门有一个隐藏了玩家背包、又写了 `player layout` 的菜单。玩家
  自己的主背包里放了一个钻石，而窗口同一格（3 行箱子下方的第一格，第 27 格）客户端读到的是页面自己的
  面包——这一条只有客户端能看见，服务端两边都是"菜单的一格里有东西"。点那一格也会带着 27 这个槽位号
  上报。
