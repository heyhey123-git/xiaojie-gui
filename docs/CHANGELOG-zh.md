...
`update title of page N …` 只改**正在看第 N 页**的玩家看到的窗口标题；停在该菜单其它页的人不受影响。
...
`the page`、`the pressed number key`；本来就有的 `event-` 形式作为别名保留，照顾已经这么写的脚本。
...
没有权限节点、没有任何选项，所以不会往 `plugins/xiaojie-gui/` 里写任何文件，那里只有脚本或服务器主人
自己放的东西。
...
- 拖拽现在真的被处理了。以前只有"恰好扫过一格"的拖拽能被看见（游戏本身会把它变成一次点击），而真正的拖拽
  ...
  那一叠（只有 `static` 菜单有真正的光标，`phantom` 窗口没有），`cancel event` 一次性拒绝整次拖拽（协议上
  也没法只应用一半）。完整机制（包括为什么拖拽只能放、不能拿）写在新的 wiki 页面《拖拽是怎么工作的》里。
...
- `with locked icons`（`build a menu` 里写 `locked icons: true`）让**布局里有图标的格子**变成菜单自己的：点击、
  Shift 点击、数字键、副手交换、丢出、双击收集都拿不走也放不进，碰到这些格子的拖拽会被整次拒绝。这是
  `static` 菜单的开关：`phantom` 窗口本来就什么都动不了。空着的格子仍然属于玩家，所以"商店"和"背包"是同一个
  机制；锁定也**不会**阻止格子回调——商店的"购买"回调照常触发，而商品
...
- 一页可以做成**列表页**：把一个列表映射给一个 key（`map key "L" to icon {results::*}`），那个 key 的格子就是
  这一页的结果格。`set the menu list of %menusession% to %itemstacks%` 为某个玩家一次填好它们——多余物品丢掉、
  没填满的格子清空、窗口**只更新一次**而不是每格一次——`the menu list index` 就是点到第几个结果（1 基，点到的
  不是结果格时没有值）。于是"物品浏览器"＝一个布局 + 一次调用 + 一个下标；浏览器仍然做不到的两件事（页数不能变少、
  窗口里不能打字）也因此明确写在文档里，而不是留给用户去发现。
- 玩家开着的那一份现在在脚本里统一叫**窗口**：`the menu window of player`，事件里就是 `the event-menu window`。
  `the menu session of player` 照样能用、意思完全一样，所以没有脚本需要改；类型说明也不再像一份
  数据库文档。文档那一页现在开头就讲清楚"窗口是什么"，并且明确说大多数脚本根本不用点名它——附带一张表，写清
  哪些事需要、哪些事不需要。
- 幽灵菜单写了 `hide player inventory` 之后，容器下方那 36 格**是菜单自己的空间，不属于玩家**：那里不会被镜像玩家
  的物品，脚本可以往里放图标，点那里的点击和别的格子一样会上报。这一点现在对"写"和"读"都成立——上一版改成"实时
  读取玩家那半场"时，隐藏情形下会把脚本自己刚放进去的图标读丢，这一版修好了。
- `player inventory layout` 现在叫 **`player layout`**，而且它描述的是**容器下方那 4 行本身**，不再是"玩家背包显示
  副本"的布局。那 4 行只有玩家背包被隐藏时才是菜单自己的空间，所以**写 player layout 就会把它隐藏**：一个关键词
  表达"这半场是我的"，布局也就不可能悄悄不生效。旧规则里它描述的是玩家那 4 行的副本，而玩家的真实物品会盖在上面
  ——那半场既不是玩家的也不是菜单的。只想要"下半场隐藏但空着"，仍然只写 `with hide player inventory`；在 `static`
  菜单上写 player layout 会给一句警告并且什么也不变。布局属于某一页，半场属于整个菜单，所以在任意一页写它都会
  隐藏整个菜单的这半场。这半场只能是其中一边：在这种菜单上调 `show player inventory of %menu%` 会被拒绝（并说明
  该怎么办），而显示玩家背包时，容器下方那几行只有玩家自己的物品——页面为它们摆的图标在窗口发出前就被清掉，
  不会再出现在玩家恰好空着的格子里。
- 幽灵窗口在读取玩家自己那半场时改成直接读实时库存，不再每次读取都复制一遍整个背包；这是性能审查里唯一一个
  值得点名的热点。
- `server-test/` 里多了两个可运行示例：列表页浏览器（`14-browser-list.sk`）和"关闭时存档、打开时还原"的背包
  （`15-backpack-persist.sk`）。`serverTest` 每次都会解析它们，所以不会悄悄失效——它已经抓出了第一个里的一个
  错误循环。
- `/skript reload` 会先关掉所有菜单再加载新脚本。菜单里存着"创建它的那批脚本"的回调，所以挺过一次重载的
  菜单，下一次点击执行的是已经不在任何脚本里的代码。重载后的脚本会像启动时一样重新建自己的菜单。
- `on menu close` 现在在**不是玩家自己关**的结束路径上也会触发（退出、死亡、换世界），而且触发时容器还在。
  这三条路以前会静默丢掉窗口，于是"关闭时存档"的背包会把玩家临走时带着的东西全丢；新增的单测把这四条
  路径的顺序钉住了。
- 两个"写了也不起作用"的开关现在会自己说出来：幽灵菜单上写 `with locked icons` 会警告"这个窗口里的东西本来
  就动不了，于是没有东西可锁"，而 static 菜单上调用 `hide player inventory of %menu%` 会像创建时的关键字那样
  警告。两者的值仍然会被存下来（脚本可以读回来），警告拦住的是"以为窗口因此变了"这件事。
...
- 点击、槽位对齐、标题、物品**类型**现在都由**真实客户端**覆盖（`./gradlew clientTest`）：一个 Minecraft
  ...
  到达真实客户端"，而不是"26.2 客户端看到的就是这些字节"。拖拽和 `locked icons` 在这一版已经纳入这一层
  （bot 直接发原始拖拽包，mineflayer 造不出来），所以仍未覆盖的是客户端侧的幽灵物品与两个客户端同时对照。
...
- 页面只能通过 `insert page` 增加（创建菜单时给的 `layout` 就是第一页的简写），`with page` 只决定 `open menu`
  先显示哪一页；没有第三种写法 —— 一页 = 一个布局 + 一个标题，这是有意为之。
