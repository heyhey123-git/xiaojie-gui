 ...
 */
class PacketHelper : IPacketHelper {

    companion object {
        val instance: IPacketHelper by lazy { PacketHelper() }
    }
...
//    fun getCurrentContainerId(player: Player): Int
     ...
     */
    fun generateNextContainerId(player: Player): Int

    //resource: container_close
    fun sendContainerClose(player: Player)

    //resource: open_screen
    fun sendOpenScreen(player: Player, windowId: Int, windowType: LayoutType, title: Component)

    // resource: container_set_content
    fun sendContainerSetContent(player: Player, windowId: Int, items: Array<ItemStack?>)

    // resource: container_set_slot
    fun sendContainerSetSlot(player: Player, windowId: Int, slot: Int, item: ItemStack?)
