...
//   - examples that differ, including the element having none where the file has one. The publisher writes
//     those too now, so a difference here is a publish run that has not happened yet; it is reported, with
//     both texts, and does not fail this run. Only the official examples are compared: an example a reader
//     submitted is theirs, sits beside ours, and no run can settle it.
...
/**
 * The official examples of one element, as the text the site stores.
 *
 * Only the official ones: an example a reader submitted sits beside ours and is not part of what the
 * publisher owns, so counting it would report a difference no run can settle. Blank lines at either end are
 * the one thing normalized, the same way the publisher compares them.
 */
const examplesOf = async (id) =>
  list(await get('/syntaxexample/?syntax=' + id))
    .filter((example) => example.official_example)
    .map((example) => example.example_code ?? '')
const exampleText = (code) => String(code ?? '').replace(/\r\n/g, '\n').replace(/^\s*\n/, '').replace(/\n\s*$/, '').trimEnd()
const siteExampleText = (examples) => exampleText(examples.join('\n'))
...
try {
  ...
  for (const [jsonId, entry] of document.entries) {
    ...
    if (siteExampleText(siteExamples) !== ourExampleText(entry)) differs.push('examples')
  ...
  section('examples that differ from the file', realExamples, ({ entry, row }) => '`' + entry.title + '` (id ' + row.id + ')')
  ...
  // Examples are the publisher's job now, and a difference there is a run that has not happened yet, so it
  // does not fail this: everything else means the site is not what this version documents, or holds
  // something this version does not.
  const problems = missing.length + drifted.filter(({ differs }) => differs.some((difference) => difference !== 'examples')).length + dropped.length
